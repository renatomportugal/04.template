...
width = 500;
margin = 50;
letterheight = (height - (2 * margin)) / alphabet.length;
letterwidth = (width - (2 * margin)) / 3;
...
var valueline = d3.line()
    .x(function(d,i) { return i * letterwidth; })
    .y(function(d,i) { 
      
      var randomVariation = Math.random() * letterheight * 0.5;
      
      return (l(d) * letterheight) + randomVariation - (letterheight/2);
...
for(var a=0; a<4; a++) {
  ...
  var letterbox = letters.selectAll("circle.letterbox")
                    ...
                    .on("click", function(d,i){
                        ...
                        var letter = d3.select(this).attr("data-letter");
                        var pos = d3.select(this).attr("data-pos");
                      
                      var obj = {p: pos, l: letter};
  ...
  if(input) {
    ...
    for(var i=0; i<data.length; i++){
      ...
      for(var j=0; j<settings.length; j++){
        ...
        if(data[i][settings[j]['p']] == settings[j]['l']) {
          cont++
        }

      }

      if(cont == settings.length) newdata.push(data[i]);
...
var line = lines.selectAll("g.line").data(newdata);
