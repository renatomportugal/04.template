/*
 *
 * @(#) $Id: animation.js,v 1.36 2013/05/19 11:27:10 mlemos Exp $
 *
 */
...
ML.Animation.Effects = {
		...
		this.start = function()
		{
			...
			if(e)
			{
				...
				if(effect.method === 'circle')
				{
					...
					&& Math.abs(distance - this.gapDistance - this.lineWidth) < 0.5);
				}
				else
				{
					...
					|| effect.method === 'double-underline')
					{
						...
						while(--iterationLimit
						&& Math.abs(distance - this.gapDistance) < 0.5);
	...
	ProgressNumber: function(effect)
	{
		...
		var formatNumber = function(value)
		{
			...
			if(!effect.thousandsSeparator
			|| effect.thousandsSeparator.length === 0)
			{
				return value;
			}
			formatted = '';
