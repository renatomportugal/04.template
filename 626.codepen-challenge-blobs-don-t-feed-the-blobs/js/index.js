...
var colors = randomColor({ luminosity: 'light', count: 120 });
...
var $allBlobs = void 0;
spawnBlobs = function spawnBlobs(amount) {
...
//	Spawn Cookies
spawnCookie = function spawnCookie(pos) {
	$food = $('.food').append(cookie);
	var randomRotation = 'rotate(' + randomBetween(0, 360) + 'deg)';
...
setInterval(function () {
	$allBlobs.each(function (index, blob) {
		var blobPos = {
		...
		$('.cookie').each(function (i, food) {
			var foodPos = {
				x: $(food).position().left + $(food).width() / 2,
				y: $(food).position().top + $(food).height() / 2 };


			var xDiff = foodPos.x - blobPos.x;
			var yDiff = foodPos.y - blobPos.y;
			var distance = Math.sqrt(yDiff * yDiff + xDiff * xDiff);
			var direction = { x: xDiff / distance, y: yDiff / distance };
