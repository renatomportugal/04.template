...
const SVG = opt => {
	...
	///////////////////////////////////////////////////////////////////////
	const initSVG = opt => {
		if (opt.centerOrigin !== undefined && opt.centerOrigin === true) offset = 100;
	...
	const buttonSave = () => {
		...
		save.appendChild(document.createTextNode("Save SVG"));
		svgElem.parentElement.appendChild(save);
		const style = {
			position: "absolute",
			left: "7px",
			top: "7px",
		...
		for (let s in style) save.style[s] = style[s];
		save.addEventListener("mouseover", e => {
			e.currentTarget.style.background = "#5a5f73";
		});
